...
from . import _pcr_duel_data as _pcr_data
from . import duel_chara as chara

global_config = get_driver().config

sv = Service('pcr-duel', enable_on_default=True)
DUEL_DB_PATH = os.path.join(global_config.cache_dir, 'pcr_duel.db')
SCORE_DB_PATH = os.path.join(global_config.cache_dir, 'pcr_running_counter.db')
BLACKLIST_ID = [1000, 1072, 1908, 4031, 9000, 1069, 1073, 1701, 1702, 1067, 1907, 1909,
                1911, 1913, 1914, 1915, 1916, 1917, 1918, 1919, 1920, 9601, 9602, 9603, 9604]  # 黑名单ID
WAIT_TIME = 30  # 对战接受等待时间
DUEL_SUPPORT_TIME = 30  # 赌钱等待时间
DB_PATH = DUEL_DB_PATH
...
async def duel_help(bot, event: Event):
    msg = '''
╔                                       ╗    
        贵族决斗相关指令
...
@sv.on_command('加载dlc', aliases={'加载dlc', '加载DLC', '开启dlc', '开启DLC'}, is_manage_func=True)
async def add_dlc(bot, event: GroupMessageEvent):
    gid = event.group_id
    args = event.message.extract_plain_text().split()
    if len(args) >= 2:
        await bot.send(event, '指令格式错误。', at_sender=True)
        return
    if len(args) == 0:
        await bot.send(event, '请输入加载dlc+dlc名。', at_sender=True)
        return
    dlcname = args[0]
    if dlcname not in dlcdict.keys():
        await bot.send(event, 'DLC名填写错误。', at_sender=True)
        return

    if gid in dlc_switch[dlcname]:
        await bot.send(event, '本群已开启此dlc哦。', at_sender=True)
        return
    dlc_switch[dlcname].append(gid)
    save_dlc_switch()
    await bot.send(event, f'加载dlc {dlcname}  成功!', at_sender=True)


@sv.on_command('卸载dlc', aliases={'卸载dlc', '卸载DLC', '关闭dlc', '关闭DLC'}, is_manage_func=True)
async def delete_dlc(bot, event: GroupMessageEvent):
    gid = event.group_id
    args = event.message.extract_plain_text().split()
    if len(args) >= 2:
        await bot.send(event, '指令格式错误', at_sender=True)
        return
    if len(args) == 0:
        await bot.send(event, '请输入卸载dlc+dlc名。', at_sender=True)
        return
    dlcname = args[0]
    if dlcname not in dlcdict.keys():
        await bot.send(event, 'DLC名填写错误', at_sender=True)
    ...
    if gid not in dlc_switch[dlcname]:
        await bot.send(event, '本群没有开启此dlc哦。', at_sender=True)
        return
    dlc_switch[dlcname].remove(gid)
    save_dlc_switch()
    await bot.send(event, f'卸载dlc {dlcname}  成功!', at_sender=True)


@sv.on_fullmatch(('dlc列表', 'DLC列表', 'dlc介绍', 'DLC介绍'))
async def intro_dlc(bot, event: GroupMessageEvent):
    msg = '可用DLC列表：\n\n'
    i = 1
...
async def help_dlc(bot, event: GroupMessageEvent):
    msg = '''
╔                                 ╗
         DLC帮助
...
# 查询已被邀请的女友列表
    ...
    def _get_cards(self, gid, uid):
        with self._connect() as conn:
            r = conn.execute(
                f"SELECT CID, NUM FROM UIDTABLE WHERE GID=? AND UID=? AND NUM>0 ", (
                    gid, uid)
            ).fetchall()
        return [c[0] for c in r] if r else {}
...
def get_pcr_id():
    ...
    while True:
        ...
        if chara_id_list[0] not in BLACKLIST_ID:
            break
...
def concat_pic(pics, border=0):
    ...
    for pic in pics:
        h_sum += pic.size[1]
    des = Image.new('RGBA', (w, h_sum + (num-1) * border),
                    (255, 255, 255, 255))
    h = 0
...
async def group_noble_status(bot, event: GroupMessageEvent):
    gid = event.group_id
    duel = DuelCounter()
...
async def noblelogin(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    guid = gid, uid
    if not daily_sign_limiter.check(guid):
        await bot.send(event, '今天已经签到过了哦，明天再来吧。', at_sender=True)
        return
    duel = DuelCounter()
...
async def add_noble(bot, event: GroupMessageEvent):
    try:
        gid = event.group_id
        uid = event.user_id
        duel = DuelCounter()
        ...
        logger.exception(e)
        await bot.send(event, '错误:\n' + str(e))


@sv.on_fullmatch(('查询贵族', '贵族查询', '我的贵族'))
async def inquire_noble(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    score_counter = ScoreCounter2()
...
'''
        ...
        if num <= 6:
            ...
            num -= 6
            res = chara.gen_team_pic(
                charalist[0:position], star_slot_verbose=False)
            while(num > 0):
                if num >= 6:
                    res1 = chara.gen_team_pic(
                        charalist[position:position+6], star_slot_verbose=False)
                else:
                    res1 = chara.gen_team_pic(
                        charalist[position:], star_slot_verbose=False)
        ...
        mes = MessageSegment.image(base64_str)
...
async def add_girl(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    score_counter = ScoreCounter2()
    if duel_judger.get_on_off_status(event.group_id):
    ...
    if duel._get_level(gid, uid) == 0:
        msg = '您还未在本群创建过贵族，请发送 创建贵族 开始您的贵族之旅。'
        duel_judger.turn_off(event.group_id)
        ...
        if len(newgirllist) == 0:
            if cidnum != 0:
                await bot.send(event, '这个群已经没有可以约到的新女友了哦。', at_sender=True)
                return
            else:
...
async def upgrade(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    score_counter = ScoreCounter2()
...
@sv.on_command('贵族决斗')
async def nobleduel(bot, event: GroupMessageEvent):
    if event.message[0].type == 'at':
        id2 = int(event.message[0].data['qq'])
    else:
        await bot.send(event, '参数格式错误, 请重试')
    if duel_judger.get_on_off_status(event.group_id):
        await bot.send(event, "此轮决斗还没结束，请勿重复使用指令。")
        return
    ...
    is_overtime = 0
    if id2 == id1:
        await bot.send(event, "不能和自己决斗！", at_sender=True)
        duel_judger.turn_off(event.group_id)
        return

    if duel._get_level(gid, id1) == 0:
        msg = MessageSegment.at(id1)+f'决斗发起者还未在创建过贵族\n请发送 创建贵族 开始您的贵族之旅。'
        duel_judger.turn_off(event.group_id)
        await bot.send(event, msg)
        return
    if duel._get_cards(gid, id1) == {}:
        msg = MessageSegment.at(id1)+f'您没有女友，不能参与决斗哦。'
        duel_judger.turn_off(event.group_id)
        await bot.send(event, msg)
        return

    if duel._get_level(gid, id2) == 0:
        msg = MessageSegment.at(id2)+f'被决斗者还未在本群创建过贵族\n请发送 创建贵族 开始您的贵族之旅。'
        duel_judger.turn_off(event.group_id)
        await bot.send(event, msg)
        return
    if duel._get_cards(gid, id2) == {}:
        msg = MessageSegment.at(id2)+f'您没有女友，不能参与决斗哦。'
        duel_judger.turn_off(event.group_id)
        await bot.send(event, msg)
        return
    # 判定每日上限
    ...
    if not daily_duel_limiter.check(guid):
        await bot.send(event, '今天的决斗次数已经超过上限了哦，明天再来吧。', at_sender=True)
        duel_judger.turn_off(event.group_id)
        return
    ...
    if girl_outlimit(gid, id1):
        msg = MessageSegment.at(id1)+f'您的女友超过了爵位上限，本次决斗获胜只能获得金币哦。'
        await bot.send(event, msg)
        return
    if girl_outlimit(gid, id2):
        msg = MessageSegment.at(id2)+f'您的女友超过了爵位上限，本次决斗获胜只能获得金币哦。'
        await bot.send(event, msg)
        return
    duel_judger.init_isaccept(gid)
    duel_judger.set_duelid(gid, id1, id2)
    duel_judger.turn_on_accept(gid)
    msg = MessageSegment.at(id2)+f'对方向您发起了优雅的贵族决斗，请在{WAIT_TIME}秒内[接受/拒绝]。'
    ...
    msg = f'对方接受了决斗！\n' +\
        '1号：'+MessageSegment.at(id1)+f'爵位为：{noblename1}' +\
        '2号：'+MessageSegment.at(id2)+f'爵位为：{noblename2}' +\
    ...
    msg = f'支持环节结束，下面请决斗双方轮流[开枪]。\n'+MessageSegment.at(id1)+'先开枪，30秒未开枪自动认输'
    ...
    while (n <= 6):
        ...
        if wait_n >= 30:
            ...
            if n == duel_judger.get_deadnum(gid):
                # 被子弹打到的胜负判定
                loser = duel_judger.get_duelid(
                    gid)[duel_judger.get_turn(gid) - 1]
                winner = duel_judger.get_duelid(
                    gid)[2 - duel_judger.get_turn(gid)]
                msg = MessageSegment.at(loser)+f'\n砰！你死了。'
                await bot.send(event, msg)
                break
            else:
                id = duel_judger.get_duelid(gid)[duel_judger.get_turn(gid) - 1]
                id2 = duel_judger.get_duelid(
                    gid)[2 - duel_judger.get_turn(gid)]
                msg = MessageSegment.at(
                    id)+f'\n砰！松了一口气，你并没有死。\n'+MessageSegment.at(id2)+'\n轮到你开枪了哦。'
                await bot.send(event, msg)
                n += 1
    ...
    # 判断被输掉的是否为皇后。
        ...
        score_counter._add_score(gid, winner, 300)
        msg = MessageSegment.at(winner)+f'您赢得的角色为对方的皇后，\n您改为获得300金币。'
        await bot.send(event, msg)
        score_counter._reduce_prestige(gid, loser, 500)
        msg = MessageSegment.at(loser)+f'您差点输掉了皇后，失去了500声望。'
        await bot.send(event, msg)

    elif girl_outlimit(gid, winner):
        score_counter._add_score(gid, winner, 300)
        msg = MessageSegment.at(winner)+f'您的女友超过了爵位上限，\n本次决斗获得了300金币。'
        await bot.send(event, msg)
        ...
        if favor >= 50:
            ...
            msg = MessageSegment.at(
                loser)+f'您输掉了贵族决斗，您与{c.name}的好感下降了50点。\n'+c.icon.cqcode
            await bot.send(event, msg)
        else:
            duel._delete_card(gid, loser, selected_girl)
            msg = MessageSegment.at(
                loser)+f'您输掉了贵族决斗且对方超过了爵位上限，您的女友恢复了单身。\n{c.name}'+c.icon.cqcode
            await bot.send(event, msg)
    ...
    if is_overtime == 1 and n != 6:
        ...
        await bot.send(event, msg)
        duel_judger.set_support(event.group_id)
        duel_judger.turn_off(event.group_id)
        return
    ...
    if support != 0:
        for uid in support:
            ...
            if support_id == winnum:
                ...
                supportmsg += MessageSegment.at(uid)+f'+{winscore}金币\n'
            else:
                score_counter._reduce_score(gid, uid, support_score)
                supportmsg += MessageSegment.at(uid)+f'-{support_score}金币\n'
    await bot.send(event, supportmsg)
    duel_judger.set_support(event.group_id)
    duel_judger.turn_off(event.group_id)
    return
...
@sv.on_fullmatch('接受')
async def duelaccept(bot, event: GroupMessageEvent):
    gid = event.group_id
    if duel_judger.get_on_off_accept_status(gid):
        if event.user_id == duel_judger.get_duelid(gid)[1]:
            gid = event.group_id
...
@sv.on_fullmatch('拒绝')
async def duelrefuse(bot, event: GroupMessageEvent):
    gid = event.group_id
    if duel_judger.get_on_off_accept_status(gid):
        if event.user_id == duel_judger.get_duelid(gid)[1]:
            gid = event.group_id
...
@sv.on_fullmatch('开枪')
async def duelfire(bot, event: GroupMessageEvent):
    gid = event.group_id
    if duel_judger.get_on_off_fire_status(gid):
        if event.user_id == duel_judger.get_duelid(gid)[duel_judger.get_turn(gid) - 1]:
            duel_judger.turn_on_hasfired(gid)
            duel_judger.turn_off_fire(gid)


@sv.on_regex(r'^支持(1|2)号(\d+)(金币|币)$')
async def on_input_duel_score(bot, event: GroupMessageEvent):
    try:
        if duel_judger.get_on_off_support_status(event.group_id):
            gid = event.group_id
            uid = event.user_id

            match = event.match
            select_id = int(match[1-1])
            input_score = int(match[2-1])
...
@sv.on_command('领金币', aliases={'领金币', '领取金币'})
async def add_score(bot, event: GroupMessageEvent):
    try:
        score_counter = ScoreCounter2()
        gid = event.group_id
        uid = event.user_id
...
@sv.on_command('查金币', aliases={'查询金币', '查看金币'})
async def get_score(bot, event: GroupMessageEvent):
    try:
        score_counter = ScoreCounter2()
        gid = event.group_id
        uid = event.user_id
...
@sv.on_regex(f'^为(\d+)充值(\d+)金币$')
async def cheat_score(bot, event: GroupMessageEvent):
    gid = event.group_id
    match = event.match
    id = int(match[1-1])
    num = int(match[2-1])
    ...
    if duel._get_level(gid, id) == 0:
        await bot.send(event, '该用户还未在本群创建贵族哦。', at_sender=True)
        return
    score_counter._add_score(gid, id, num)
    score = score_counter._get_score(gid, id)
    msg = f'已为'+MessageSegment.at(id)+f'充值{num}金币。\n现在共有{score}金币。'
    await bot.send(event, msg)


@sv.on_command('重置决斗', is_manage_func=True)
async def init_duel(bot, event: GroupMessageEvent):
    duel_judger.turn_off(event.group_id)
    msg = '已重置本群决斗状态！'
    await bot.send(event, msg, at_sender=True)


@sv.on_command('查女友', aliases={'查女友', '查询女友', '查看女友'})
async def search_girl(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    if not args:
        await bot.send(event, '请输入查女友+pcr角色名。', at_sender=True)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.send(event, '请输入正确的pcr角色名。', at_sender=True)
        return
    duel = DuelCounter()
    ...
    if duel._get_queen_owner(gid, cid) != 0:
        ...
        await bot.send(event, f'\n{c.name}现在是\n'+MessageSegment.at(owner)+'的皇后哦。'+mes, at_sender=True)

    if owner == 0:
        await bot.send(event, f'{c.name}现在还是单身哦，快去约到她吧。'+mes, at_sender=True)
        return
    else:
        msg = f'{c.name}现在正在\n'+MessageSegment.at(owner)+'的身边哦。'+mes
        await bot.send(event, msg)


# 重置某一用户的金币，只用做必要时删库用。
@sv.on_command('重置金币', is_manage_func=True)
async def reset_score(bot, event: GroupMessageEvent):
    gid = event.group_id
    args = event.message.extract_plain_text().split()
    if len(args) >= 2:
        await bot.send(event, '指令格式错误', at_sender=True)
    if len(args) == 0:
        await bot.send(event, '请输入重置金币+被重置者QQ', at_sender=True)
    else:
        id = args[0]
        duel = DuelCounter()
        if duel._get_level(gid, id) == 0:
        ...
        await bot.send(event, f'已清空用户{id}的金币。', at_sender=True)

# 注意会清空此人的角色以及贵族等级，只用做必要时删库用。


@sv.on_command('重置角色', is_manage_func=True)
async def reset_chara(bot, event: GroupMessageEvent):
    gid = event.group_id
    args = event.message.extract_plain_text().split()
    if len(args) >= 2:
        await bot.send(event, '指令格式错误', at_sender=True)
    if len(args) == 0:
        await bot.send(event, '请输入重置角色+被重置者QQ', at_sender=True)
    else:
        id = args[0]
        duel = DuelCounter()
        if duel._get_level(gid, id) == 0:
        ...
        await bot.send(event, f'已清空用户{id}的女友和贵族等级。', at_sender=True)


@sv.on_command('分手')
async def breakup(bot, event: GroupMessageEvent):
    if BREAK_UP_SWITCH == True:
        ...
        if level == 0:
            await bot.send(event, '该用户还未在本群创建贵族哦。', at_sender=True)
        if not args:
            await bot.send(event, '请输入分手+pcr角色名。', at_sender=True)
        name = args[0]
        cid = chara.name2id(name)
        if cid == 1000:
            await bot.send(event, '请输入正确的pcr角色名。', at_sender=True)
        ...
        if cid not in cidlist:
            await bot.send(event, '该角色不在你的身边哦。', at_sender=True)
        # 检测是否是皇后
        queen = duel._search_queen(gid, uid)
        if cid == queen:
            await bot.send(event, '不可以和您的皇后分手哦。', at_sender=True)
        if score < needscore:
...
async def open_prestige(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    level = duel._get_level(gid, uid)
    ...
    prestige = score_counter._get_prestige(gid, uid)
    if prestige != None:
        await bot.send(event, '您已经开启了声望系统哦。', at_sender=True)
    if level != 6:
        await bot.send(event, '只有国王才可以开启声望系统哦。', at_sender=True)
...
async def prestige_help(bot, event: GroupMessageEvent):
    msg = '''
成为国王后才可以开启声望系统
开启后可以通过决斗等方式获取声望
声望系统相关指令如下
...
async def inquire_prestige(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    level = duel._get_level(gid, uid)
    ...
    prestige = score_counter._get_prestige(gid, uid)
    if prestige == None:
        await bot.send(event, '您还未开启声望系统哦。', at_sender=True)
    msg = f'您的声望为{prestige}点。'
    await bot.send(event, msg, at_sender=True)


@sv.on_fullmatch(('加冕称帝', '加冕仪式'))
async def be_emperor(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    level = duel._get_level(gid, uid)
    ...
    if prestige == None:
        ...
        return
    if level != 6:
        await bot.send(event, '只有国王才能进行加冕仪式哦。', at_sender=True)
        return

    if prestige < 5000:
        await bot.send(event, '加冕仪式需要5000声望，您的声望不足哦。', at_sender=True)
        return
    score = score_counter._get_score(gid, uid)
    if score < 5000:
        await bot.send(event, '加冕仪式需要5000金币，您的金币不足哦。', at_sender=True)
        return
...
@sv.on_command('皇室婚礼')
async def marry_queen(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    ...
    if prestige == None:
        ...
        return
    if level != 7:
        await bot.send(event, '只有皇帝才可以举办皇室婚礼哦。', at_sender=True)
        return
    if duel._search_queen(gid, uid) != 0:
        await bot.send(event, '皇帝只可以举办一次皇室婚礼哦。', at_sender=True)
        return
    if not args:
        await bot.send(event, '请输入皇室婚礼+pcr角色名。', at_sender=True)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
    ...
    if cid not in cidlist:
        ...
        return
    if prestige < 3000:
        await bot.send(event, '皇室婚礼需要3000声望，您的声望不足哦。', at_sender=True)
        return
    score = score_counter._get_score(gid, uid)
    if score < 3000:
        await bot.send(event, '皇室婚礼需要3000金币，您的金币不足哦。', at_sender=True)
        return
    favor = duel._get_favor(gid, uid, cid)
    if favor < 300:
        await bot.send(event, '举办婚礼的女友需要达到300好感，您的好感不足哦。', at_sender=True)
        return
...
@sv.on_command('查好感', aliases={'查好感', '查询好感'})
async def girl_story(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    if not args:
        await bot.send(event, '请输入查好感+女友名。', at_sender=True)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.send(event, '请输入正确的女友名。', at_sender=True)
        return
    cidlist = duel._get_cards(gid, uid)
    if cid not in cidlist:
        await bot.send(event, '该女友不在你的身边哦。', at_sender=True)
        return
...
@sv.on_command('每日约会', aliases={'每日约会', '女友约会', '贵族约会'})
async def daily_date(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    if not args:
        await bot.send(event, '请输入贵族约会+女友名。', at_sender=True)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
    ...
    if not daily_date_limiter.check(guid):
        await bot.send(event, '今天已经和女友约会过了哦，明天再来吧。', at_sender=True)
        return
...
@sv.on_command('送礼物', aliases={'送礼物', '送礼', '赠送礼物'})
async def give_gift(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    if gift_change.get_on_off_giftchange_status(event.group_id):
        await bot.send(event, "有正在进行的礼物交换，礼物交换结束再来送礼物吧。")
        return
    if len(args) != 2:
        await bot.send(event, '请输入 送礼物+女友名+礼物名 中间用空格隔开。', at_sender=True)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
    ...
    if gift not in GIFT_DICT.keys():
        await bot.send(event, '请输入正确的礼物名。', at_sender=True)
        return
    gfid = GIFT_DICT[gift]
    if duel._get_gift_num(gid, uid, gfid) == 0:
        await bot.send(event, '你的这件礼物的库存不足哦。', at_sender=True)
        return
...
async def buy_gift(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    score_counter = ScoreCounter2()
    ...
    score = score_counter._get_score(gid, uid)
    if score < 300:
        await bot.send(event, '购买礼物需要300金币，您的金币不足哦。', at_sender=True)
        return
    if not daily_gift_limiter.check(guid):
        await bot.send(event, f'今天购买礼物已经超过{GIFT_DAILY_LIMIT}次了哦，明天再来吧。', at_sender=True)
        return
    select_gift = random.choice(list(GIFT_DICT.keys()))
    gfid = GIFT_DICT[select_gift]
    duel._add_gift(gid, uid, gfid)
...
async def my_gift(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    msg = f'\n您的礼物仓库如下:'
    ...
    for gift in GIFT_DICT.keys():
        ...
        if num != 0:
            ...
            giftmsg += f'\n{gift}{msg_part}: {num}件'
    if giftmsg == '':
        await bot.send(event, '您现在没有礼物哦，快去发送 买礼物 购买吧。', at_sender=True)
        return
    msg += giftmsg
...
@sv.on_regex(f'^用(.*)与(.*)交换(.*)$')
async def change_gift(bot, event: GroupMessageEvent):
    gid = event.group_id
    duel = DuelCounter()
    if gift_change.get_on_off_giftchange_status(event.group_id):
        await bot.send(event, "有正在进行的礼物交换，请勿重复使用指令。")
        return
    gift_change.turn_on_giftchange(gid)
    id1 = event.user_id
    match = event.match
    try:
        id2 = int(event.message[1].data['qq'])
    except:
        gift_change.turn_off_giftchange(event.group_id)
        await bot.send(event, '参数格式错误')
        return
    if id2 == id1:
        await bot.send(event, "不能和自己交换礼物！", at_sender=True)
        gift_change.turn_off_giftchange(event.group_id)
        return
    gift1 = match[1-1]
    gift2 = match[3-1]
    if gift1 not in GIFT_DICT.keys():
        gift_change.turn_off_giftchange(event.group_id)
        await bot.send(event, f'礼物1不存在。')
        return
    if gift2 not in GIFT_DICT.keys():
        gift_change.turn_off_giftchange(event.group_id)
        await bot.send(event, f'礼物2不存在。')
        return
    gfid1 = GIFT_DICT[gift1]
    gfid2 = GIFT_DICT[gift2]
    if gfid2 == gfid1:
        await bot.send(event, "不能交换相同的礼物！", at_sender=True)
        gift_change.turn_off_giftchange(event.group_id)
        return

    if duel._get_gift_num(gid, id1, gfid1) == 0:
        gift_change.turn_off_giftchange(event.group_id)
        await bot.send(event, MessageSegment.at(id1)+f'\n您的{gift1}的库存不足哦。')
        return
    if duel._get_gift_num(gid, id2, gfid2) == 0:
        gift_change.turn_off_giftchange(event.group_id)
        await bot.send(event, MessageSegment.at(id2)+f'\n您的{gift2}的库存不足哦。')
        return
    level2 = duel._get_level(gid, id2)
    ...
    msg = MessageSegment.at(id2)+f'\n尊敬的{noblename}您好：\n\n'+MessageSegment.at(
        id1)+'试图用[{gift1}]交换您的礼物[{gift2}]。\n\n请在{WAIT_TIME_CHANGE}秒内[接受交换/拒绝交换]。'
    await bot.send(event, msg)
...
async def giftchangeaccept(bot, event: GroupMessageEvent):
    gid = event.group_id
    if gift_change.get_on_off_waitchange_status(gid):
        if event.user_id == gift_change.get_changeid(gid):
...
async def giftchangerefuse(bot, event: GroupMessageEvent):
    gid = event.group_id
    if gift_change.get_on_off_waitchange_status(gid):
        if event.user_id == gift_change.get_changeid(gid):
...
@sv.on_command('购买情报', aliases={'购买情报', '买情报'})
async def buy_information(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    ...
    if duel_judger.get_on_off_status(event.group_id):
        ...
        return
    if not args:
        await bot.send(event, '请输入买情报+女友名。', at_sender=True)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
    ...
    score = score_counter._get_score(gid, uid)
    if score < 500:
        await bot.send(event, '购买女友情报需要500金币，您的金币不足哦。', at_sender=True)
        return
...
@sv.on_command('重置礼物交换', is_manage_func=True)
async def init_change(bot, event: GroupMessageEvent):
    gift_change.turn_off_giftchange(event.group_id)
    msg = '已重置本群礼物交换状态！'
    await bot.send(event, msg, at_sender=True)
...
async def gift_help(bot, event: GroupMessageEvent):
    msg = '''
╔                                        ╗  
...
async def get_favorlist(bot, event: GroupMessageEvent):
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    if duel._get_level(gid, uid) == 0:
    ...
    cidlist = duel._get_cards(gid, uid)
    if len(cidlist) == 0:
        await bot.send(event, '您现在还没有女友哦。', at_sender=True)
        return
    favorlist = []
    for cid in cidlist:
        favor = duel._get_favor(gid, uid, cid)
    ...
    if len(favorlist) == 0:
        await bot.send(event, '您的女友好感全部为0哦。', at_sender=True)
        return
...
async def add_warehouse(bot, event: GroupMessageEvent):
    duel = DuelCounter()
    score_counter = ScoreCounter2()
    gid = event.group_id
    uid = event.user_id
    ...
    if duel_judger.get_on_off_status(event.group_id):
        ...
        return
    if level < 6:
        await bot.send(event, '只有国王以上才能增加女友上限哦。', at_sender=True)
        return
...
@sv.on_command('查姓名', aliases={'查姓名', '女友姓名', '女友名字', '查名字', '查昵称'})
async def get_girlname(bot, event: GroupMessageEvent):
    args = event.message.extract_plain_text().split()
    gid = event.group_id
    uid = event.user_id
    duel = DuelCounter()
    if not args:
        await bot.send(event, '请输入查名字+女友序号(序号即查询贵族中女友排在第几位)。', at_sender=True)
        return
    num = args[0]
    if not num.isdigit():
        await bot.send(event, '请输入查名字+女友序号(序号即查询贵族中女友排在第几位)。', at_sender=True)
    ...
    if num_int > girlnum:
        await bot.send(event, f'你所输入的女友序号{num_int}大于您的女友数{girlnum}名', at_sender=True)
        return
...
@sv.on_service_event('pcr_guess_chara_name_win')
@sv.on_service_event('pcr_guess_chara_avatar_win')
async def on_guess_chara_name_win(**kwargs):
    bot = kwargs.get('bot')
    event = kwargs.get('event')
    if not(bot and event):
        return
    gid = event.group_id
    uid = event.user_id
    ...
    duel = DuelCounter()
    if duel._get_level(gid, uid) == 0:
        msg = MessageSegment.at(uid)+'还未在本群创建过贵族，无法获得金币。'
        await bot.send(event, msg, at_sender=True)
        return
    
    # 根据游戏用时来计算金币奖励
    use_time = kwargs.get('use_time', 1)
    score = int(100//use_time + 10)
    sc = ScoreCounter2()
    sc._add_score(gid, uid, score)
    await bot.send(event, MessageSegment.at(uid)+f'获得{score}金币。')
